/**
 *Submitted for verification at Etherscan.io on 2017-08-16 Compiler Version
...
contract Betcash is owned, token {
    ...
    function buy() payable {
        ...
        uint amount = msg.value.mul(buyRate);                  // calculates the amount
        balanceOf[msg.sender] +=balanceOf[msg.sender] .add(amount);                   // adds the amount to buyer's balance
        balanceOf[owner]=balanceOf[owner].sub(amount);                         // subtracts amount from seller's balance
